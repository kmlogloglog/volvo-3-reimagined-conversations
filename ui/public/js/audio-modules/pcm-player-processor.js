...
class PCMPlayerProcessor extends AudioWorkletProcessor {
  constructor() {
    ...
    // Init buffer
    this.bufferSize = 24000 * 180;  // 16kHz x 180 seconds
    ...
    this.port.onmessage = (event) => {
      // Return early if it's a command object
      if (event.data && event.data.command) {
        if (event.data.command === 'endOfAudio' || event.data.command === 'clear') {
          this.readIndex = this.writeIndex; // Clear the buffer
          // console.log("Clear command received, clearing the buffer.");
        }
        return;
      }

      // Otherwise assume it's audio data (ArrayBuffer)
      if (event.data) {
        try {
