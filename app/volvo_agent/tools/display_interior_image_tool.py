# Import the FunctionTool class from the ADK.
    ...
    try:
        ...
        for entry in data:
            if entry.get("interior", "").lower() == target:
                logger.info(f"Found exact match for '{interior_name}': {entry.get('interior')}")
...
def display_interior_image(car_model: CarModel, interior_type: InteriorType) -> dict:
    ...
    interior_name = interior_type.value if hasattr(interior_type, 'value') else interior_type
    ...
    if not image_url:
        return {
            "agent_context": f"Could not find an image for {interior_name}."
        }
