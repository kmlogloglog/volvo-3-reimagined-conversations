...
class CarConfiguration(BaseModel):
    ...
    model: Literal["EX30", "EX60", "EX90"] = Field(
        ..., description="The car model."
    )
    exterior: Optional[str] = Field(
        default=None, description="The selected exterior color for the car configuration (e.g. vapour_grey, onyx_black)."
    )
    interior: Optional[str] = Field(
        default=None, description="The selected interior for the car configuration (e.g. nordico_charcoal, nappa_cardamom)."
    )
    wheels: Optional[str] = Field(
        default=None, description="The selected wheels type for the car configuration (e.g. 20, 21, 22Y)."
    )
    ...
    def set_defaults_and_validate(self) -> "CarConfiguration":
        ...
        if not model_data:
             raise ValueError(f"Invalid model '{model_key}'. Valid options: {list(CAR_CONFIGS.keys())}")
        ...
        if self.exterior:
             ...
             if self.exterior not in valid_exteriors:
                 ...
                 raise ValueError(f"Invalid exteriors '{self.exterior}' for model {self.model}. Valid options: {list(valid_exteriors.keys())}")

        # 5. Validate Interiors
        if self.interior:
            valid_interiors = model_data.get("interiors", {})
            if self.interior not in valid_interiors:
                 raise ValueError(f"Invalid interior '{self.interior}' for model {self.model}. Valid options: {list(valid_interiors.keys())}")

        # 6. Validate Wheels
        if self.wheels:
            valid_wheels = model_data.get("wheels", {})
            if str(self.wheels) not in valid_wheels:
                 raise ValueError(f"Invalid wheels '{self.wheels}' for model {self.model}. Valid options: {list(valid_wheels.keys())}")
