...
- Comments must always be on their **own line** — never inline (trailing after code on the same line).
- Only add comments where the code is non-obvious. Do not comment self-explanatory code.
- **No decorated comments** — never use patterns like `// --- Section ---`, `// *** Title ***`, `// ========`. Plain `// text` only.
- **Never delete comments that explain config values, magic numbers, or non-obvious constants** — if they were inline, convert them to a new line above the code instead of removing them.
...
> **CRITICAL: When the user says "app" in a frontend context, they always mean `ui/app/` — NOT the root-level `app/` folder (which is the Python backend). All frontend file creation, edits, and reviews must be scoped to `ui/`. Never create or modify files outside `ui/` unless the user explicitly asks to work on the backend.**
